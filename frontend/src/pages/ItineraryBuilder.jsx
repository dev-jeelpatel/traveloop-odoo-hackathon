      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="space-y-4">
          ...
          <div className="glass-card p-5 space-y-3">
            ...
            {cities.length > 0 && (
              <div className="glass-card divide-y divide-white/5 max-h-48 overflow-y-auto">
                {cities.map(city => (
            ...
            {selectedCity && (
              <div className="flex items-center gap-2">
                <div className="badge badge-primary flex-1">{selectedCity.name}, {selectedCity.country}</div>
          ...
          {stops.length === 0 ? (
            <div className="glass-card p-10 text-center">
              ...
              <p className="text-white/40">No stops yet. Search a city above to begin.</p>
            </div>
          ) : (
            <div className="space-y-3">
              {stops.map((stop, idx) => (
                <div key={stop.id} className="glass-card p-4">
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-2">
                      <div className="w-7 h-7 rounded-full bg-primary-600/30 border border-primary-500/50 flex items-center justify-center text-xs font-bold text-primary-300">
                        {idx + 1}
                      </div>
                      <div>
                        <p className="font-semibold text-white">{stop.city?.name}</p>
                        <p className="text-xs text-white/40">Day {stop.dayNumber} · {stop.city?.country}</p>
                      </div>
                    </div>
                    <button onClick={() => removeStop(stop.id)} className="btn-icon w-7 h-7 text-coral-400 hover:bg-coral-500/10">
                  ...
                  {stop.stopActivities?.length > 0 && (
                    <div className="space-y-1.5 mb-3">
                      {stop.stopActivities.map(sa => (
                        <div key={sa.id} className="flex items-center justify-between bg-white/5 rounded-lg px-3 py-2">
                          <div className="flex items-center gap-2">
                            <Clock className="w-3 h-3 text-white/30" />
                            <span className="text-xs text-white/70">{sa.activity?.name}</span>
                            <span className="badge badge-primary">{sa.activity?.category}</span>
                          </div>
                          <button onClick={() => removeActivity(stop, sa.activityId)} className="text-white/20 hover:text-coral-400 transition-colors">
                            <X className="w-3 h-3" />
                          </button>
                        </div>
                      ))}
                  ...
                  {selectedCity?.id === stop.cityId && activities.length > 0 && (
                    <div className="border-t border-white/10 pt-3">
                      <p className="text-xs text-white/40 mb-2">Quick add activities:</p>
                      <div className="flex flex-wrap gap-1.5">
                        {activities.slice(0, 8).map(act => (
                          <button key={act.id} onClick={() => addActivity(stop, act)}
                            className="text-xs px-2 py-1 rounded-lg bg-primary-600/20 text-primary-300 border border-primary-500/30 hover:bg-primary-600/40 transition-colors">
                            + {act.name}
                          </button>
                        ))}
                      </div>
                  ...
                  {stop.city?.id !== selectedCity?.id && (
                    <button onClick={() => setSelectedCity(stop.city)} className="text-xs text-white/30 hover:text-primary-400 transition-colors mt-1">
        ...
        {/* Right: Map */}
        <div className="h-[500px] lg:h-auto glass-card overflow-hidden sticky top-4">
