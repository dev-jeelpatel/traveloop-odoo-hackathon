...
import { useEffect } from "react"; import api from "../../lib/api"; import { Plus, Search, Edit2, Star } from "lucide-react";
const CATS = ["Adventure","Hiking","Beaches","Food Tours","Cultural","Snow","Monsoon","Wildlife","Shopping","Religious"];
export default function ManageActivities() {
  const [acts, setActs] = useState([]); const [q, setQ] = useState(""); const [cat, setCat] = useState("All");
  ...
  return (
    <motion.div className="space-y-6" initial={{opacity:0,y:16}} animate={{opacity:1,y:0}}>
      <div className="flex items-center justify-between"><div><h1 className="page-title">Activities</h1><p className="page-subtitle">{acts.length} activities listed</p></div><button className="btn-primary"><Plus className="w-4 h-4"/>Add Activity</button></div>
      <div className="flex gap-2 flex-wrap">
        {["All",...CATS].map(c=><button key={c} onClick={()=>setCat(c)} className="px-4 py-2 rounded-2xl text-sm font-semibold transition-all" style={{background:cat===c?"linear-gradient(135deg,#2E7D6B,#3D9B85)":"rgba(255,255,255,0.6)",color:cat===c?"#fff":"#6B7280",border:"1px solid rgba(124,154,126,0.2)"}}>{c}</button>)}
      </div>
